...
export type AppointmentType = 'registration' | 'selection-tests' | 'accident-report' | 'responsibility-waiver' | 'other';
...
export const getAppointmentBadge = (type: AppointmentType) => {
  switch (type) {
    ...
    case 'selection-tests': return 'bg-blue-500';
    case 'accident-report': return 'bg-red-500';
    case 'responsibility-waiver': return 'bg-purple-500';
...
export const isTimeSlotAvailable = (appointments: Appointment[], date: Date, time: string) => {
...
export const getAvailableTimeSlotsForDate = (appointments: Appointment[], date: Date) => {
...
export const isAppointmentInHour = (appointment: Appointment, date: Date, hour: number) => {
  return appointment.date.getHours() === hour && isSameDay(appointment.date, date);
};

// Get all appointments for a specific hour and date
export const getAppointmentsForHourAndDate = (appointments: Appointment[], date: Date, hour: number) => {
