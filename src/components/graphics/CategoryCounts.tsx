...
const CategoryCounts: React.FC<CategoryCountsProps> = ({
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text("Daring Brussels Academy", 105, 20, { align: "center" });
      pdf.setFontSize(16);
      ...
      // Sauvegarder le PDF
      pdf.save("daring-brussels-academy-stats.pdf");
    } catch (error) {
  ...
  const dbfTeams = teams.filter((t) => /^DBF\s/i.test(t.teamName));
  const dbTeams = teams.filter(
    (t) =>
      /(DARING BRUSSELS|Daring Brussels|RWDM)/i.test(t.teamName) &&
      !/^DBF\s/i.test(t.teamName)
  );

  const stats: Record<"DBF" | "DB" | "EBFA", CategoryStats> = {
    DBF: {
      teams: dbfTeams.length,
      players: dbfTeams.reduce((sum, t) => sum + t.playerCount, 0),
    },
    DB: {
      teams: dbTeams.length,
      players: dbTeams.reduce((sum, t) => sum + t.playerCount, 0),
    },
    EBFA: {
    ...
    <Card className="mb-8 mt-8">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button
          ...
          onClick={generatePDF}
          disabled={loading || isPdfGenerating} // Désactive pendant la génération
        >
          {isPdfGenerating ? (
            <>
